...
export class NeteaseUserAgentInterceptor implements IInterceptor {

  onRequest(request: AxiosRequestConfig): AxiosRequestConfig {
    const ua = request.headers.ua;
    if (typeof ua === 'undefined') {
      const index = Math.floor(Math.random() * USER_AGENT_LIST.length);
      request.headers['User-Agent'] = USER_AGENT_LIST[index];
    } else if (ua === 'mobile') {
      const index = Math.floor(Math.random() * 7);
      request.headers['User-Agent'] = USER_AGENT_LIST[index];
    } else if (ua === 'pc') {
      const index = Math.floor(Math.random() * 5) + 8;
      request.headers['User-Agent'] = USER_AGENT_LIST[index];
    } else {
